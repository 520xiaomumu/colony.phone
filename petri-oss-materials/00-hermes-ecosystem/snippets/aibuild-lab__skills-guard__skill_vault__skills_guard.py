/* NET-NEW? 净增:skill扫描
 * source: https://github.com/aibuild-lab/skills-guard/blob/main/skill_vault/skills_guard.py
 * stars: 9 bytes: 37759
 */
...
This file is INTENTIONALLY a verbatim port to maintain compatibility with
upstream Hermes. Do not modify the threat patterns directly; submit
improvements upstream (https://github.com/NousResearch/hermes-agent) and
re-port the resulting file.

The only deviations from the upstream verbatim source are:
  1. This attribution header (added).
  2. The original module docstring is preserved below this header.
  3. No code changes. Identical regex patterns, identical helpers, identical
...
Usage:
    from skill_vault.skills_guard import scan_skill, should_allow_install, format_scan_report
